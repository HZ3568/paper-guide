...
def verify_node(state: AgentState) -> dict:
    ...
    try:
        ...
        ).invoke([HumanMessage(content=VERIFY_PROMPT.format(
    ...
    unsupported_claims = list(dict.fromkeys(
        [*verdict.unsupported_claims, *claim_report["unmapped_claims"]]
    ))
    claim_failed = bool(
        claim_report["invalid_evidence_ids"] or claim_report["unmapped_claims"]
    )
    if verdict.grounded and not unsupported_claims and not citation_failed and not claim_failed:
