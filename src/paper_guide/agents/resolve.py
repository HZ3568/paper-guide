...
- 单数指代只有在上下文中能唯一 grounding 时才改写；零个或多个候选都交 clarify，绝不瞎猜。
"""
...
from paper_guide.agents.clarify import (
    ...
    has_singular_reference,
    replace_singular_reference,
...
# 当助手因本地库无相关论文而提议「搜索并入库」时，记录一条待用户确认的搜索意图。
# kind="search" 只表示待确认事项的类型，不表示 Resolve 会直连 Search 节点。
# 用户确认后，Resolve 将 query 取出为 resolved_query 并清空该记录；后续仍由 Supervisor 路由。
# 用户拒绝或提出新任务时，Resolve 直接清空该记录。
_SEARCH_OFFER_MARKERS = ("要我去搜", "要我搜", "搜索并入库", "去搜一批", "搜一批")


def answer_offers_search(text: str) -> bool:
    """用固定话术标记判断助手回复是否提出了「搜索并入库」建议。

    本函数只返回检测结果，不修改 state，也不触发搜索。
    调用方在返回 True 时可用 make_search_pending() 写入 pending_action。
    """
...
def make_search_pending(question: str) -> dict:
    ...
    kind 用于 Resolve 识别待确认事项的类型；query 是用户确认后恢复的完整指令。
    Resolve 将 query 写入 resolved_query 后仍进入 Supervisor，由 Supervisor 决定是否路由到 Search。
    """
...
def resolve_node(state: AgentState) -> dict:
    ...
    入口解析。路径优先级：
      ...
      B) 搜索提议的确认（好啊/不用了）
      C) 自包含问题 → 透传（0 LLM）
      D) 含指代/追问 → 引用解析（1 LLM）；单数指代无法唯一 grounding → 澄清
    ...
    if decision:
        push_progress(_tid, "❓ 指代无法可靠唯一解析，需澄清")
        print(f"[Resolve] 指代需澄清（{decision.reason}，{len(decision.options)} 个候选）")
        return {**base, "pending_action": build_clarify_pending(decision, raw)}

    # 单数指代的唯一候选已被 judge_reference 的 grounding 校验确认。
    # 不使用模型给出的裸改写，避免它暗中补充或替换错误对象。
    candidates = grounded_reference_candidates(msgs, result)
    if has_singular_reference(raw) and len(candidates) == 1:
        resolved = replace_singular_reference(raw, candidates[0].entity)
        push_progress(_tid, f"✍️ 指代消解：{raw[:12]} → {resolved[:40]}")
        print(f"[Resolve] 基于证据改写 '{raw}' → '{resolved}'")
