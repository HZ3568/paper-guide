...
只覆盖「可可靠观测」的 5 类澄清信号，`low_relevance` 暂缓（检索层尚未透出可比 rerank 分数）：
  ...
  - unresolved_reference  ：单数指代在历史中没有可核验候选，不能猜测
...
ClarifyReason = Literal[
    "ambiguous_reference", "unresolved_reference", "multiple_papers", "entity_conflict", "exact_title_not_found",
]
...
def has_singular_reference(raw: str) -> bool:
...
def grounded_reference_candidates(
    messages: list[BaseMessage], result: ResolveResult,
) -> list[ReferentCandidate]:
    """返回有历史原文证据的去重候选，绝不把模型裸枚举当作事实。"""
    unique: dict[str, ReferentCandidate] = {}
    for candidate in result.candidates:
        if _grounded(candidate, messages):
            key = _norm(candidate.entity)
            if key:
                unique.setdefault(key, candidate)
    return list(unique.values())


def replace_singular_reference(raw: str, entity: str) -> str:
    """只替换第一个单数指代，实体必须已通过 grounding 校验。"""
    return re.sub("|".join(map(re.escape, _SINGULAR_REF)), entity, raw, count=1)


def judge_reference(raw: str, messages: list[BaseMessage], result: ResolveResult) -> ClarifyDecision | None:
    """
    单数指代必须可靠地唯一落到历史证据上：
    - 0 个 grounded 候选：无法可靠解析，要求用户直接说明对象；
    - 2 个及以上 grounded 候选：有歧义，展示选项；
    - 恰好 1 个：无需澄清，交由 Resolve 做确定性替换。
    """
    if not has_singular_reference(raw):
        return None

    cands = grounded_reference_candidates(messages, result)
    if not cands:
        return ClarifyDecision(
            reason="unresolved_reference",
            question="我不确定你说的“它”具体指什么。请直接说出论文、方法或对象的名称。",
        )
    if len(cands) == 1:
        return None
    ...
    return ClarifyDecision(
        ...
        options=[
            ClarifyOption(
                ...
                resolved_query=replace_singular_reference(raw, c.entity),
            )
...
def resolve_reference_llm(raw: str, messages: list[BaseMessage], llm: ChatOpenAI | None = None) -> ResolveResult:
    """
    resolve 阶段那唯一一次改写调用，扩成结构化「改写 + 候选枚举」。异常时原样返回、无候选，
    由调用方把单数指代作为未解析内容交给澄清，而不是信任模型猜测。
    ...
    prompt = REFERENCE_RESOLUTION_PROMPT.format(indexed_history=indexed, raw_query=raw)
    try:
...
_REFERENCE_REPLY_PREFIXES = ("我说的是", "我指的是", "指的是", "是", "就是")
_UNKNOWN_REFERENCE_REPLIES = ("不知道", "不清楚", "不确定", "忘了")
...
def parse_clarification_selection(reply: str, pending: dict) -> Selection:
    ...
    # 2) 无候选的指代澄清：用户直接补充对象名，确定性填回原问题。
    if pending.get("reason") == "unresolved_reference":
        if _looks_like_new_task(reply_s):
            return Selection(action="reprocess", resolved_query=reply_s)
        if not reply_s or any(k in reply_s for k in _UNKNOWN_REFERENCE_REPLIES):
            return _reclarify_or_fallback(attempts)
        entity = reply_s
        for prefix in _REFERENCE_REPLY_PREFIXES:
        ...
        if entity:
            return Selection(
                action="select",
                resolved_query=replace_singular_reference(
                    pending.get("original_query", ""), entity,
                ),
            )
        return _reclarify_or_fallback(attempts)

    # 3) 序号选择（越界不默认、不崩，继续澄清）
    ...
    # 5) 像新的完整任务 → 清空旧澄清并按新任务处理
    if _looks_like_new_task(reply_s):
        return Selection(action="reprocess", resolved_query=reply_s)

    # 6) 含糊回应 → 继续澄清（受 attempts 护栏约束）
    return _reclarify_or_fallback(attempts)
...
def clarify_node(state: dict) -> dict:
    ...
    - 无候选指代澄清：保留 pending_action，下一轮接收用户直接补充的对象名。
    - 兜底：渲染后清空 pending_action，避免继续循环。
    """
    ...
    if pending.get("reason") == "fallback":
