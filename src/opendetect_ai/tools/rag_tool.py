...
# ── 工具函数 ───────────────────────────────────────────────────
def _normalise_identifier_text(value: str) -> str:
    """为稳定 ID 归一化文本；不用于展示或正文内容。"""
    value = unicodedata.normalize("NFKC", value or "").casefold()
    return re.sub(r"\s+", " ", value).strip()


def _make_paper_id(title: str, arxiv_id: str, authors: str = "") -> str:
    """生成逻辑论文 ID：优先使用事实来源明确的 arXiv ID。"""
    normalised_arxiv = _normalize_arxiv_id(arxiv_id) if arxiv_id else ""
    if normalised_arxiv:
        return f"arxiv:{normalised_arxiv}"
    identity = "\x1f".join((
        _normalise_identifier_text(title),
        _normalise_identifier_text(authors),
    ))
    return f"local:{hashlib.sha256(identity.encode('utf-8')).hexdigest()[:24]}"


def _file_sha256(file_path: str) -> str:
    """流式计算 PDF 哈希，避免把整个文件一次性读入内存。"""
...
def _make_document_id(paper_id: str, document_hash: str) -> str:
    """同一逻辑论文的不同 PDF 文件拥有不同 document_id。"""
    return f"{paper_id}:pdf:{document_hash[:24]}"


def _make_chunk_id(document_id: str, locator: str) -> str:
    """用文档身份和块位置生成不依赖全局 chunk 顺序的稳定 ID。"""
    digest = hashlib.sha256(f"{document_id}\x1f{locator}".encode("utf-8")).hexdigest()
...
def _paper_already_ingested(vectorstore: Chroma, arxiv_id: str, title: str) -> bool:
    ...
    try:
        ...
        required_ids = ("paper_id", "document_id", "chunk_id", "content_hash")
        return bool(metadatas) and all(
            int((meta or {}).get("parser_version", 0)) >= _PARSER_VERSION
            and int((meta or {}).get("metadata_version", 0)) >= _METADATA_VERSION
            and (meta or {}).get("embedding_model") == OPENDETECT_EMBED_MODEL
            and all((meta or {}).get(key) for key in required_ids)
...
def _build_pdf_documents(
    ...
    *,
    ...
    """构造摘要、正文、表格和图片块，并赋予分层稳定 ID。"""
    pages = _parse_pdf_layout(file_path)
    documents: list[Document] = []
    ids: list[str] = []
    chunk_idx = 0
    document_hash = _file_sha256(file_path)
    paper_id = _make_paper_id(title, arxiv_id, authors)
    document_id = _make_document_id(paper_id, document_hash)
    base_metadata = {
        "paper_id": paper_id,
        ...
        "embedding_model": OPENDETECT_EMBED_MODEL,
    }

    def append_document(content: str, locator: str, metadata: dict) -> None:
        chunk_id = _make_chunk_id(document_id, locator)
    ...
    if abstract_text:
        content = f"## Abstract\n\n{abstract_text}"
        append_document(content, "abstract", {
            "page": 0,
            "page_chunk_idx": -1,
            "chunk_idx": chunk_idx,
            "element_type": "abstract",
            "element_number": "",
            "element_title": "Abstract",
            "reference_pages": "",
            "reference_count": 0,
    ...
    for page in pages:
        ...
        for page_chunk_idx, chunk in enumerate(_splitter.split_text(page["text"])):
            append_document(chunk, f"text:p{page_number}:c{page_chunk_idx}", {
                "page": page_number,
        ...
        for table_idx, table in enumerate(page["tables"]):
            ...
            append_document(content, f"table:p{page_number}:i{table_idx}", {
                "page": page_number,
                "page_chunk_idx": -1,
                "chunk_idx": chunk_idx,
                "element_type": "table",
        ...
        for figure_idx, figure in enumerate(page["figures"]):
            ...
            append_document(content, f"figure:p{page_number}:i{figure_idx}", {
                "page": page_number,
                "page_chunk_idx": -1,
                "chunk_idx": chunk_idx,
                "element_type": "figure",
...
def _document_counts(documents: list[Document]) -> dict[str, int]:
    counts = {"text": 0, "abstracts": 0, "tables": 0, "figures": 0}
...
def ingest_paper(
    ...
    Args:
        ...
        authors:    作者列表（可选）
        published:  发表日期（可选）
        abstract:   搜索来源返回的论文摘要（可选，单独作为摘要块入库）
...
def list_ingested_papers() -> list[dict]:
    ...
    for meta in result.get("metadatas", []):
        ...
        paper_id = meta.get("paper_id") or title
