...
def _paper():
    return PaperMeta(title="Paper X", pdf_url="http://x/p.pdf",
                     arxiv_id="1234.5678", abstract="Paper abstract", retry_count=0)
...
def test_ingest_recovers_official_pdf_url_from_arxiv_id(monkeypatch) -> None:
    ...
    assert tool.payloads[0]["abstract"] == ""


def test_ingest_forwards_search_abstract(monkeypatch) -> None:
    paper = _paper()

    class CaptureTool(_FakeTool):
        def __init__(self):
            super().__init__([{"status": "ok", "chunks": 2}])
            self.payloads = []
    ...
    ingest_mod.ingest_node(state)

    assert tool.payloads[0]["abstract"] == "Paper abstract"
