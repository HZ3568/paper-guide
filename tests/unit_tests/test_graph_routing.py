...
def test_verify_rejects_contradictory_grounded_flag_with_unsupported_claim(monkeypatch) -> None:
    """模型字段互相矛盾时，不能因为 grounded=true 就放行。"""
    class FakeRunnable:
        def invoke(self, _messages):
            return verify_mod._Verdict(
                grounded=True,
    ...
    monkeypatch.setattr(verify_mod, "OPENDETECT_VERIFY", True)
    monkeypatch.setattr(verify_mod, "_get_llm", lambda: FakeLLM())
    out = verify_mod.verify_node({
        "rag_answer": "Transformer 使用自注意力。（来源：Attention Is All You Need，第 2 页）",
        "rag_context": [{"title": "Attention Is All You Need", "content": "self-attention"}],
    })
    assert out["verification"]["status"] == "warning"
    assert out["verification"]["grounded"] is False
    assert out["verification"]["unsupported_claims"] == ["该方法提升了 20%"]


def test_verify_collects_unmapped_claim_even_if_model_omits_it(monkeypatch) -> None:
    class FakeRunnable:
        def invoke(self, _messages):
            return verify_mod._Verdict(
                grounded=True,
                ...
                unsupported_claims=[],
                claim_evidence=[verify_mod._ClaimEvidence(
                    claim="该方法提升了 20%", evidence_ids=[], supported=False,
                )],
            )
    ...
    monkeypatch.setattr(verify_mod, "_get_llm", lambda: FakeLLM())
    out = verify_mod.verify_node({
        "rag_answer": "该方法提升了 20%。（来源：Paper A，第 1 页）",
...
def test_verify_receives_query_and_full_evidence_and_replaces_draft(monkeypatch) -> None:
    ...
    assert "主题相关不等于直接支撑" in captured["prompt"]
    assert "数字、年份、数据集、指标、比较、因果关系" in captured["prompt"]
