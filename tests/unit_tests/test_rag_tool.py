...
def test_build_pdf_documents_preserves_page_numbers(tmp_path) -> None:
    ...
    documents, ids, pages = rag_tool._build_pdf_documents(
        ...
        source="local",
        abstract="A paper about vision transformers.",
    )

    assert pages == 2
    text_documents = [
        item for item in documents if item.metadata["element_type"] == "text"
    ]
    assert {item.metadata["page"] for item in text_documents} == {1, 2}
    assert len(ids) == len(set(ids)) == len(documents)
    assert ids == [item.metadata["chunk_id"] for item in documents]
    assert {item.metadata["paper_id"] for item in documents} == {"arxiv:1234.5678"}
    assert len({item.metadata["document_id"] for item in documents}) == 1
    assert all(
        item.metadata["content_hash"]
        == hashlib.sha256(item.page_content.encode("utf-8")).hexdigest()
        for item in documents
    )

    abstract = next(
        item for item in documents if item.metadata["element_type"] == "abstract"
    )
    assert abstract.metadata["page"] == 0
    assert abstract.metadata["content_source"] == "search_metadata"
    assert "vision transformers" in abstract.page_content

    rebuilt, rebuilt_ids, _ = rag_tool._build_pdf_documents(
        str(pdf_path),
        title="Paper",
    ...
    assert [item.metadata["document_id"] for item in rebuilt] == [
...
def test_parser_version_controls_reingestion() -> None:
    class FakeVectorstore:
        def __init__(
            ...
            parser_version,
            metadata_version=0,
            embedding_model=rag_tool.OPENDETECT_EMBED_MODEL,
        ):
    ...
    assert (
        rag_tool._paper_already_ingested(
            FakeVectorstore(
                rag_tool._PARSER_VERSION, rag_tool._METADATA_VERSION
            ),
            "id",
            "Paper",
        )
    ...
    assert (
        rag_tool._paper_already_ingested(
            FakeVectorstore(
                rag_tool._PARSER_VERSION,
                rag_tool._METADATA_VERSION,
                "old-embedding-model",
            ),
            "id",
            "Paper",
